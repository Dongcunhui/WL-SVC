...
class Model_all(torch.nn.Module):
    def __init__(self, wacelet_trainable_set, train_step):
        ...
        self.bitplane_number = 9
        self.bitplane_encodenumber = 4
        # 生成第一级高频
        self.preditct_wavelet_encode_H1 = Model_train_predict()
        self.update_wavelet_encode_H1 = Model_train_update()
        self.space_entropy_H1 = Model_space_entropy(wacelet_trainable_set, self.bitplane_number, train_step)
        ...
        self.space_entropy_H2 = Model_space_entropy(wacelet_trainable_set, self.bitplane_number, train_step)
        ...
        self.space_entropy_H3 = Model_space_entropy(wacelet_trainable_set, self.bitplane_number, train_step)
        self.preditct_wavelet_decode_H3 = Model_train_predict()  
        self.update_wavelet_decode_H3 = Model_train_update()

        # 生成低频帧
        self.space_entropy_L = Model_space_entropy(wacelet_trainable_set, self.bitplane_number, train_step)
    ...
    def forward(self, frame, mask, mv, grid_up, grid_org, train, alpha_list):
        ...
        bits_H3 = 0
        bits_L = 0
        for part_bitplane in range(0, self.bitplane_encodenumber+1):
            H1 = torch.cat(H1_list[:4], 0)
            rec_H1, bits_H1_tmp, wavelet_scale_H1 = self.space_entropy_H1(H1, train, part_bitplane)  # 重建时域第二帧
            bits_H1 = bits_H1 + bits_H1_tmp
            with torch.no_grad():
                rec_H1_nogrid, bits_H1_nogrid, wavelet_scale_H1_nogrid = self.space_entropy_H1(H1_list[4], train, part_bitplane)
            H2 = torch.cat(H2_list[:2], 0)
            rec_H2, bits_H2_tmp, wavelet_scale_H2 = self.space_entropy_H2(H2, train, part_bitplane)  # 重建时域第二帧
            bits_H2 = bits_H2 + bits_H2_tmp
            with torch.no_grad():
                rec_H2_nogrid, bits_H2_nogrid, wavelet_scale_H2_nogrid = self.space_entropy_H2(H2_list[2], train, part_bitplane)    
            rec_H3, bits_H3_tmp, wavelet_scale_H3 = self.space_entropy_H3(H3_list[0], train, part_bitplane)  # 重建时域第二帧
            bits_H3 = bits_H3 + bits_H3_tmp
            with torch.no_grad():
                rec_H3_nogrid, bits_H3_nogrid, wavelet_scale_H3_nogrid = self.space_entropy_H3(H3_list[1], train, part_bitplane) 
            rec_L, bits_L_tmp, wavelet_scale_L = self.space_entropy_L(L3_list[0], train, part_bitplane)  # 重建时域第1帧
            bits_L = bits_L + bits_L_tmp
            with torch.no_grad():
                rec_L_nogrid, bits_L_nogrid, wavelet_scale_L_nogrid = self.space_entropy_L(L3_list[1], train, part_bitplane)  # 重建时域第1帧
                # rec_L_nogrid = self.image_enhance_L(rec_L_nogrid)
            
        # 重建第三级update
        L3_rec_update_list = []
        i = 0
        L_tmp = self.update_wavelet_decode_H3(rec_H3,
                                              -mv[:, 128 + 8 + 4 * i:128 + 8 + 4 * i + 2, :, :],
                                              mask[:, 64 + 4 + i * 2, :, :],
                                              rec_H3,
                                              -mv[:, 128 + 8 + 4 * i + 2:128 + 8 + 4 * i + 4, :, :],
                                              mask[:, 64 + 4 + i * 2 + 1, :, :], grid_up,
                                              grid_org)
        L3 = rec_L/beta_level - L_tmp/alpha_level
        ...
        with torch.no_grad():
            ...
            L_tmp = self.update_wavelet_decode_H3(rec_H3,
                                                  ...
                                                  mask[:, 64 + 4 + i * 2 + 1, :, :], grid_up,
                                                  grid_org)
            L3 = rec_L_nogrid/beta_level - L_tmp/alpha_level
            L3_rec_update_list.append(L3)
        ...
        predict_john, _, predict_cnn = self.preditct_wavelet_decode_H3(L3_rec_update_list[0],
                                                                       ...
                                                                       grid_up,
                                                                       grid_org)
        H_tmp = rec_H3/alpha_level + predict_cnn
        # H_tmp = self.image_enhance_H3(H_tmp, predict_cnn, rec_H3/alpha_level)
