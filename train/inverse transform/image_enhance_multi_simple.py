...
class PostProcessing(torch.nn.Module):
    def __init__(self, internal_channel=16):
        ...
        body_predict = [ResBlock(self.internal_channel) for _i in range(3)]
        self.body_predict = torch.nn.Sequential(*body_predict)
        body_resi = [ResBlock(self.internal_channel) for _i in range(1)]
        ...
        self.conv2 = torch.nn.Conv2d(in_channels=self.internal_channel, out_channels=self.internal_channel, kernel_size=3, stride=1, padding=0)
        self.conv3 = torch.nn.Conv2d(in_channels=self.internal_channel, out_channels=self.internal_channel, kernel_size=3, stride=1, padding=0)
        self.conv4 = torch.nn.Conv2d(in_channels=self.internal_channel, out_channels=1, kernel_size=3, stride=1, padding=0)
    ...
    def forward(self, x, predict, resi):
        ...
        conv2 = self.conv2(self.padding(self.relu(conv1)))
        conv3 = self.conv3(self.padding(self.relu(conv2)))
        conv4 = self.conv4(self.padding(self.relu(conv3)))
